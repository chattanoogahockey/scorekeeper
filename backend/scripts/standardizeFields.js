...
/**
 * Field Standardization Migration Script
 * 
...
async function standardizeFields() {
  ...
  try {
    ...
    for (const penalty of penalties) {
      ...
      if (updated) {
        await penaltiesContainer.item(penalty.id, penalty.id).replace(penalty);
        penaltiesFixed++;
        migrationLog.push({
          container: 'penalties',
          id: penalty.id,
    ...
    for (const goal of goals) {
      ...
      if (updated) {
        await goalsContainer.item(goal.id, goal.id).replace(goal);
        goalsFixed++;
        migrationLog.push({
          container: 'goals',
          id: goal.id,
    ...
    for (const shot of shots) {
      ...
      if (updated) {
        await shotsContainer.item(shot.id, shot.id).replace(shot);
        shotsFixed++;
        migrationLog.push({
